...
// Normal test scenario with native SOL (wrapped SOL)
#[tokio::test]
async fn test_native() {
    let mut helper = TestHelper::new(20).await;

    let admin = generate_signer();

    // new_vault_type (prepare)

    let SetupForVaultTypeWithNativeResult {
        vault_type,
        mint,
        pool,
        reserve,
        decimals: _,
    } = helper.setup_for_vault_type_with_native(&admin).await;
    ...
    let sync_native_ix =
        spl_token::instruction::sync_native(&spl_token::id(), &user_token_account).unwrap();

    let mut transaction = solana_sdk::transaction::Transaction::new_with_payer(
        &[transfer_ix, sync_native_ix],
        Some(&helper.payer.pubkey()),
    );
    ...
    let bh = helper
        ...
        .await
        .unwrap();
    transaction.sign(&[&helper.payer], bh);
    helper
        .context
        .banks_client
    ...
    let result = helper.close_vault_type(vault_type, &admin, pool, reserve).await;
